...
void set_signal_name(t_signal* t_signal_ptr, char* name){
	int string_length;
...
char signal_value_to_char(signal_value value){
	if(value==STD_U)
		return 'U';
	if(value==STD_0)
                return '0';
...
void display_signal_values(t_signal* signal_list_head){
	printf("# ** NOTE:\n ");
	while(signal_list_head!=NULL){
		printf("%s = \'%c\'  \n ",(signal_list_head->name),signal_value_to_char(signal_list_head->value));
