...
void
attach_port_list (t_entity * entity_ptr, t_port * port_head,
		  t_port * port_tail, char *port_type,
		  signal_value port_value)
{
  if (port_type[0] == 'i')
	...
	{
	  if (trie_search (port_head->name, entity_ptr->in_port_trie_root) !=
	      NULL
	      || trie_search (port_head->name,
			      entity_ptr->out_port_trie_root) != NULL)
	    {
	      printf ("** ERROR: duplicate port name\n");
	      exit (EXIT_FAILURE);
	    }
	  trie_insert ((void *) port_head, port_head->name,
		       entity_ptr->in_port_trie_root);
...
void
insert_port_set (t_entity * entity_ptr)
{
  ...
  if (scanf (" %1[oO]", temp_string_end_buffer) == 1)
      ...
      if (scanf (" %1[sS]", temp_string_end_buffer) == 1)
	{
	  ...
	  attach_port_list (entity_ptr, port_head, port_tail, "o",
			    port_value);
	}
    }
      ...
      if (scanf (" %1[sS]", temp_string_end_buffer) == 1)
	{
	  ...
	  attach_port_list (entity_ptr, port_head, port_tail, "i",
			    port_value);
	}
    }
  else
...
void
input_ports (t_entity * entity_ptr)
{
