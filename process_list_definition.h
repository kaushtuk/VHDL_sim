...
void
set_process_name (t_process * t_process_ptr, char *name)
{
  int string_length;
...
void
display_process_names (t_process * process_list_head)
{
  printf ("# ** NOTE: \n");
  while (process_list_head != NULL)
    {
      printf ("%s -> %dns \n ", process_list_head->name,
	      process_list_head->after_time_stamp);
