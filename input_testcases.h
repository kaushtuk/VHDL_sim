...
trie* input_signal_to_trie(t_signal* signal_list_head,t_entity* parent_entity_ptr){
	...
	while(scanf(" %1[sS]",temp_string_end_buffer)==1){
		...
		if(scanf("%*1[iI]%*1[gG]%*1[nN]%*1[aA]%*1[lL]%1[ \n\t]",temp_string_end_buffer)<1){
		...
		if(trie_search(signal_name,parent_entity_ptr->in_port_trie_root)!=NULL || trie_search(signal_name,parent_entity_ptr->out_port_trie_root)!=NULL){
			printf("** ERROR:port name and signal name are same\n");
			exit(EXIT_FAILURE);
		}
		trie_insert((void*)temp_signal_list_head,signal_name,signal_trie_root);
...
void add_trigger(t_signal* signal_ptr,t_process* process_ptr){
	t_trigger* trigger_list_head = signal_ptr->triggered_processes;
	if(trigger_list_head==NULL){
		signal_ptr->triggered_processes=(t_trigger*)malloc(sizeof(t_trigger));
...
void* create_parse_tree(trie* signal_trie_root,trie* in_port_trie_root,t_process* parent_process,node_type* root_type){
	//new
	t_token* head=(t_token*)malloc(sizeof(t_token));
	t_token* tail= tokenizer(head,parent_process,signal_trie_root,in_port_trie_root);
...
int input_processes(t_process* process_list_head,t_signal** signal_tail_address,trie* signal_trie_root,t_entity* parent_entity_ptr,trie* entity_trie_root,trie* architecture_trie_root){
	char process_name[50],triggering_signal_name[50],driven_signal_name[50],after_choice[5],temp_string_end_buffer[2];
	t_process* previous_process_ptr = NULL;
	int is_process_list_empty=1,process_list_recreate=0;
	//parsed
	if(scanf(" %*1[bB]%*1[eE]%*1[gG]%*1[iI]%*1[nN]%1[ \n\t]",temp_string_end_buffer)<1){
	...
	while(strcmpci(process_name,"end")!=0){
		...
		if(scanf(" %1[pP]",temp_string_end_buffer)==1){
			//process
			if(scanf("%*1[rR]%*1[oO]%*1[cC]%*1[eE]%*1[sS]%1[sS]",temp_string_end_buffer)<1){
				printf("** ERROR: invalid concurrent statement\n");
				exit(EXIT_FAILURE);
			}
			...
			t_signal* temp_triggering_signal=(t_signal*)(trie_search(triggering_signal_name,signal_trie_root));
			if(temp_triggering_signal==NULL){
				t_port* temp_triggering_port=(t_port*)(trie_search(triggering_signal_name,parent_entity_ptr->in_port_trie_root));
				if(temp_triggering_port==NULL || temp_triggering_port->signal_ptr==NULL){
					printf("** ERROR: invalid triggering signal name\n");
					exit(EXIT_FAILURE);
				}
			...
			if(scanf(" %*1[)] %*1[bB]%*1[eE]%*1[gG]%*1[iI]%*1[nN]%1[ \n\t]",temp_string_end_buffer)<1){
				printf("** ERROR: invalid triggering signal name\n");
				exit(EXIT_FAILURE);
			}
			//signal assignment
			...
			process_list_head->driven_signal_ptr=(t_signal*)(trie_search(driven_signal_name,signal_trie_root));
			if(process_list_head->driven_signal_ptr==NULL){
				t_port* temp_driven_port=(t_port*)(trie_search(driven_signal_name,parent_entity_ptr->out_port_trie_root));
				if(temp_driven_port==NULL || temp_driven_port->signal_ptr==NULL){
					printf("** ERROR: invalid process driven signal name\n");
					exit(EXIT_FAILURE);
				}
				process_list_head->driven_signal_ptr=temp_driven_port->signal_ptr;
			}
			...
			process_list_head->parse_tree_root_ptr=create_parse_tree(signal_trie_root,parent_entity_ptr->in_port_trie_root,process_list_head,&(process_list_head->root_type));

			//report statement
			if(scanf(" %1[rR]",temp_string_end_buffer)==1){
				if(scanf("%*1[eE]%*1[pP]%*1[oO]%*1[rR]%*1[tT]%1[ \n\t]",temp_string_end_buffer)<1){
					printf("** ERROR:invalid report statement\n");
					exit(EXIT_FAILURE);
				}
				process_list_head->report_head_ptr = input_report_statement(signal_trie_root,parent_entity_ptr->in_port_trie_root);
			}
			else{
				process_list_head->report_head_ptr = NULL;
			}
			//end process statement
			if(scanf(" %*1[eE]%*1[nN]%*1[dD] %*1[pP]%*1[rR]%*1[oO]%*1[cC]%*1[eE]%*1[sS]%*1[sS] %1[;]",temp_string_end_buffer)<1){
			...
			if(scanf("%*1[nN]%*1[tT]%*1[iI]%*1[tT]%*1[yY]%*1[ \n\t] %*1[wW]%*1[oO]%*1[rR]%*1[kK] %1[.]",temp_string_end_buffer)<1){
				printf("** ERROR: invalid concurrent statement\n");
				exit(EXIT_FAILURE);
			}
			//limitation
			if(is_process_list_empty==1){
				...
				t_process* temp_recreate_process=(t_process*)malloc(sizeof(t_process));
				process_list_head->next = temp_recreate_process;
				previous_process_ptr = entity_instantiation(process_name,process_list_head,signal_tail_address,signal_trie_root,parent_entity_ptr,entity_trie_root,architecture_trie_root);
				process_list_head = temp_recreate_process;
				//printf("** ERROR: There should be a process before an entity instantiation\n");
				//exit(EXIT_FAILURE);
			}
			else{
				previous_process_ptr = entity_instantiation(process_name,previous_process_ptr,signal_tail_address,signal_trie_root,parent_entity_ptr,entity_trie_root,architecture_trie_root);
			}
			is_process_list_empty=0;			
		}
		else{
	...
	if(scanf("%*1[ \n\t] %*1[aA]%*1[rR]%*1[cC]%*1[hH]%*1[iI]%*1[tT]%*1[eE]%*1[cC]%*1[tT]%*1[uU]%*1[rR]%*1[eE] %1[;]",temp_string_end_buffer)<1){
...
t_signal* port_signal_instantiate(t_signal* signal_list_head,t_entity* parent_entity_ptr){
	//out port
...
t_testcase_handle input_testcase(t_entity* parent_entity_ptr,trie* entity_trie_root,trie* architecture_trie_root){
	...
	signal_list_head = port_signal_instantiate(signal_list_head,parent_entity_ptr);

	//main data structure---------------------------------------------------------------------------------------------
	//signal input
	trie * signal_trie_root=input_signal_to_trie(signal_list_head,parent_entity_ptr);

	//signal sentinal removal
	if(strcmp(testcase_handle_1.signal_list_head->name,"SIGNAL_SENTINAL")==0){
	...
	if(signal_list_head == NULL){
		process_list_recreate = input_processes(process_list_head,&signal_list_head,signal_trie_root,parent_entity_ptr,entity_trie_root,architecture_trie_root);
		testcase_handle_1.signal_list_head = signal_list_head;
	}
	else
		process_list_recreate = input_processes(process_list_head,&signal_list_head,signal_trie_root,parent_entity_ptr,entity_trie_root,architecture_trie_root);
	...
	if(process_list_recreate==1){
		...
		testcase_handle_1.process_list_head=(testcase_handle_1.process_list_head)->next;
		free(temp_pro);
	}

	//process sentinal removal
	if(strcmp(testcase_handle_1.process_list_head->name,"PROCESS_SENTINAL")==0){
