#ifndef TOKENIZER_H
#define TOKENIZER_H
typedef enum{TOKEN_OPERATOR,TOKEN_SIGNAL,TOKEN_OPENING_BRACE,TOKEN_CLOSING_BRACE,TOKEN_SENTINAL} token_type;
...
bool strcmpci(char const *a, char const *b){
        int i;
...
t_token* tokenizer(t_token* token_list_head,t_process* parent_process,trie* signal_trie_root,trie* in_port_trie_root){
	...
	while(scanf(" %1[;]",temp_string_end_buffer)<1){
		if(scanf(" %[a-zA-Z0-9_]",token_name)<1){
			...
			((t_operator_node*)(token_list_head->token_node_ptr))->type=BINARY_OPERATOR;
			((t_operator_node*)(token_list_head->token_node_ptr))->e_operator=STD_OR;
		}
			...
			token_list_head->brace_root_type=OPERATOR_NODE_TYPE;	
                        token_list_head->token_node_ptr=malloc(sizeof(t_operator_node));
                        ((t_operator_node*)(token_list_head->token_node_ptr))->type=BINARY_OPERATOR;
                        ((t_operator_node*)(token_list_head->token_node_ptr))->e_operator=STD_XOR;
                }
			...
			token_list_head->brace_root_type=OPERATOR_NODE_TYPE;
                        token_list_head->token_node_ptr=malloc(sizeof(t_operator_node));
                        ((t_operator_node*)(token_list_head->token_node_ptr))->type=BINARY_OPERATOR;
                        ((t_operator_node*)(token_list_head->token_node_ptr))->e_operator=STD_AND;
                }
		else if(strcmpci(token_name,"not")==0){
                        token_list_head->type=TOKEN_OPERATOR;
			token_list_head->brace_root_type=OPERATOR_NODE_TYPE;
                        token_list_head->token_node_ptr=malloc(sizeof(t_operator_node));
                        ((t_operator_node*)(token_list_head->token_node_ptr))->type=UNARY_OPERATOR;
                        ((t_operator_node*)(token_list_head->token_node_ptr))->e_operator=STD_NEG;
                }
		else{
			token_list_head->type=TOKEN_SIGNAL;
			token_list_head->brace_root_type=SIGNAL_NODE_TYPE;
			token_list_head->token_node_ptr=trie_search(token_name,signal_trie_root);
			if(token_list_head->token_node_ptr==NULL){
				t_port* temp_token_port=(t_port*)trie_search(token_name,in_port_trie_root);
				if(temp_token_port==NULL || temp_token_port->signal_ptr==NULL){
                                	printf("** ERROR: illegal token name\n");
                                	exit(EXIT_FAILURE);
				}
				token_list_head->token_node_ptr=(void*)(temp_token_port->signal_ptr);
                        }
		}
