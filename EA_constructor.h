...
t_architecture *
architecture_constructor (char *name, t_entity * entity_ptr)
{
  int i;
  t_architecture *architecture_ptr =
    (t_architecture *) malloc (sizeof (t_architecture));
  for (i = 0; name[i] != '\0'; i++);
  ...
  architecture_ptr->architecture_testcase_handle =
    (t_testcase_handle *) malloc (sizeof (t_testcase_handle));
