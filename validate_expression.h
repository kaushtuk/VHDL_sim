...
bool check_parentheses(t_token_dll* token_dlist){
	int paren_check=0;
...
void remove_brace_signal(t_token_dll* token_dlist){
	t_token* head=token_dlist->head;
        t_token* tail=token_dlist->tail;
	...
	while(exp_itr!=tail){
		if(exp_itr->type==TOKEN_SIGNAL){
			...
			i=exp_itr->previous;
			j=exp_itr->next;
			while(i!=head->previous && j!=tail && i->type==TOKEN_OPENING_BRACE && j->type==TOKEN_CLOSING_BRACE){
				...
				t_token* temp=i;
				i=i->previous;free(temp);
				temp=j;j=j->next;free(temp);
			}
		}
...
void check_operator_LHS_RHS(t_token_dll* token_dlist){
	t_token* head=token_dlist->head;
        t_token* tail=token_dlist->tail;
	...
	while(exp_itr!=head->previous){
		if(exp_itr->type==TOKEN_OPERATOR){
			...
			if(exp_itr->next->type==TOKEN_OPERATOR && ((t_operator_node*)(exp_itr->next->token_node_ptr))->type==BINARY_OPERATOR){
                                printf("** ERROR:illegal operator RHS \n");
                                exit(EXIT_FAILURE);
                        }
			
			//LHS
			if(((t_operator_node*)(exp_itr->token_node_ptr))->type==BINARY_OPERATOR){
				if(exp_itr==head){
			...
			if(((t_operator_node*)(exp_itr->token_node_ptr))->type==UNARY_OPERATOR){
				if(exp_itr!=head){
...
void remove_brace_unary_operator(t_token_dll* token_dlist){
	t_token* head=token_dlist->head;
        t_token* tail=token_dlist->tail;
	t_token* exp_itr=tail->previous;
        while(exp_itr!=head->previous){
		if(exp_itr->type==TOKEN_OPERATOR && ((t_operator_node*)(exp_itr->token_node_ptr))->type==UNARY_OPERATOR){
...
void remove_brace_binary_operator(t_token_dll* token_dlist){
	t_token* head=token_dlist->head;
	t_token* tail=token_dlist->tail;
	t_token* exp_itr=head;
        while(exp_itr!=tail){
		if(exp_itr->type==TOKEN_OPERATOR && ((t_operator_node*)(exp_itr->token_node_ptr))->type==BINARY_OPERATOR){
                        t_token* j=exp_itr->next;
                        if(j->type==TOKEN_OPENING_BRACE)
                                j=j->closing_brace_ptr;
...
void validate_expression(t_token_dll* token_dlist){
	...
	t_token* head=token_dlist->head;
        ...
        while(exp_itr!=tail){
		if(exp_itr->type==TOKEN_SIGNAL){
			if(exp_itr->next->type==TOKEN_SIGNAL || exp_itr->next->type==TOKEN_OPENING_BRACE){
		...
		if(exp_itr->type==TOKEN_OPERATOR){
			...
			t_operator_node* temp_optr=((t_operator_node*)(exp_itr->token_node_ptr));
