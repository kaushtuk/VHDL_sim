...
void *
copy_parse_tree (void *root_ptr, node_type root_type,
		 trie * dup_signal_trie_root, trie * port_map_trie_root)
{
  if (root_type == SIGNAL_NODE_TYPE)
    {
      void *uncast_signal_ptr;
      uncast_signal_ptr =
	trie_search (((t_signal *) (root_ptr))->name, port_map_trie_root);
      if (uncast_signal_ptr == NULL)
	uncast_signal_ptr =
	  trie_search (((t_signal *) (root_ptr))->name, dup_signal_trie_root);
      else
	uncast_signal_ptr =
	  (void *) (((t_port *) uncast_signal_ptr)->signal_ptr);
      return uncast_signal_ptr;
    }
  t_operator_node *temp_optr =
    (t_operator_node *) malloc (sizeof (t_operator_node));
  ...
  if (temp_optr->type == BINARY_OPERATOR)
    {
      ...
      temp_optr->LHS =
	copy_parse_tree (((t_operator_node *) root_ptr)->LHS,
			 ((t_operator_node *) root_ptr)->LHS_type,
			 dup_signal_trie_root, port_map_trie_root);
    }
  temp_optr->RHS_type = ((t_operator_node *) root_ptr)->RHS_type;
  temp_optr->RHS =
    copy_parse_tree (((t_operator_node *) root_ptr)->RHS,
		     ((t_operator_node *) root_ptr)->RHS_type,
		     dup_signal_trie_root, port_map_trie_root);
  return (void *) temp_optr;
}

t_testcase_handle
copy_testcase (t_testcase_handle * org_testcase_ptr,
	       trie * port_map_trie_root,
	       t_signal ** parent_signal_tail_address, char *instance_name)
{
  ...
  t_signal *dup_signal_head = NULL;
  int i = 0;
  t_signal *itr_signal_head;
  ...
  while (org_signal_head != NULL)
    {
      ...
      t_trigger *dup_trigger_head = NULL;
      int j = 0;
      t_trigger *itr_trigger_head = NULL;
      while (org_trigger_head != NULL)
	{
	  if (j == 0)
	    {
	      dup_trigger_head = (t_trigger *) malloc (sizeof (t_trigger));
	      itr_trigger_head = dup_trigger_head;
	      j++;
	    }
	  else
	    {
	      itr_trigger_head->next =
	  ...
	  itr_trigger_head->scheduled_process =
	    (t_process *) trie_search (org_trigger_head->scheduled_process->
				       name, dup_process_trie_root);
	  if (itr_trigger_head->scheduled_process == NULL)
		...
		(t_process *) malloc (sizeof (t_process));
	      ...
	      trie_insert ((void *) temp_process,
			   org_trigger_head->scheduled_process->name,
			   dup_process_trie_root);
	    }
      ...
      if (trie_search (org_signal_head->name, port_map_trie_root) == NULL)
	{
	  if (i == 0)
	    {
	      ...
	      itr_signal_head = dup_signal_head;
	      i++;
	    }
	  else
	    {
	  ...
	  trie_insert ((void *) itr_signal_head, itr_signal_head->name,
		       dup_signal_trie_root);
	}

      //it also checks duplicate triggers
      if (trie_search (org_signal_head->name, port_map_trie_root) != NULL)
	{
	  t_signal *port_mapped_signal =
	    ((t_port *)
	     trie_search (org_signal_head->name,
			  port_map_trie_root))->signal_ptr;
	  //resolving port values and parent signal value to parent signal value(only for out ports)***
	  while (dup_trigger_head != NULL)
	    {
	      t_trigger *port_mapped_trigger_head =
		port_mapped_signal->triggered_processes;
	      bool found_duplicate_trigger = 0;
	      while (port_mapped_trigger_head != NULL)
		{
		  if (port_mapped_trigger_head->scheduled_process ==
		      dup_trigger_head->scheduled_process)
		    {
		...
		{
		  t_trigger *temp_trigg =
		    (t_trigger *) malloc (sizeof (t_trigger));
		  temp_trigg->scheduled_process =
		    dup_trigger_head->scheduled_process;
  ...
  //process copy
  t_process *dup_process_head = NULL;
  int k = 0;
  t_process *itr_process_head = NULL;
  while (org_process_head != NULL)
	...
	{
	  dup_process_head =
	    (t_process *) trie_search (org_process_head->name,
				       dup_process_trie_root);
	  itr_process_head = dup_process_head;
	  k++;
	}
      else
	{
	  itr_process_head->next =
	    (t_process *) trie_search (org_process_head->name,
				       dup_process_trie_root);
	...
	{
	  printf
	    ("** ERROR:A process without triggeres exits in the entity instantiation\n");
	  exit (EXIT_FAILURE);
	}
      ...
      itr_process_head->driven_signal_ptr =
	(t_signal *) trie_search (org_process_head->driven_signal_ptr->name,
				  port_map_trie_root);
      if (itr_process_head->driven_signal_ptr == NULL)
	{
	  itr_process_head->driven_signal_ptr =
	    (t_signal *) trie_search (org_process_head->driven_signal_ptr->
				      name, dup_signal_trie_root);
	}
      else
	{
	  itr_process_head->driven_signal_ptr =
	    ((t_port *) ((void *) (itr_process_head->driven_signal_ptr)))->
	    signal_ptr;
	}
      //driver copy
      add_process_driver_to_signal (itr_process_head);
      //report copy
      itr_process_head->report_head_ptr =
	copy_report_statement (org_process_head->report_head_ptr,
			       port_map_trie_root, dup_signal_trie_root);
      //parse tree copy
      itr_process_head->parse_tree_root_ptr =
	copy_parse_tree (org_process_head->parse_tree_root_ptr,
			 org_process_head->root_type, dup_signal_trie_root,
			 port_map_trie_root);
      itr_process_head->next = NULL;
...
t_process *
entity_instantiation (char *instance_name, t_process * previous_process_ptr,
		      t_signal ** parent_signal_tail_address,
		      trie * signal_trie_root, t_entity * parent_entity_ptr,
		      trie * entity_trie_root, trie * architecture_trie_root)
{
  ...
  if (scanf (" %1[(]", temp_string_end_buffer) == 1)
      ...
      if (scanf (" %[a-zA-Z0-9_]", architecture_name) < 1)
	{
      ...
      architecture_ptr =
	(t_architecture *) trie_search (architecture_name,
  ...
  if (scanf (" %1[pP]", temp_string_end_buffer) == 1)
    {
      if (scanf
	  ("%*1[oO]%*1[rR]%*1[tT] %*1[mM]%*1[aA]%*1[pP] %1[(]",
      ...
      if (scanf (" %[a-zA-Z0-9_]", signal_name) < 1)
	{
      ...
      port_ptr =
	(t_port *) trie_search (port_name, entity_ptr->in_port_trie_root);
      if (port_ptr == NULL)
	{
	  port_ptr =
	    (t_port *) trie_search (port_name,
				    entity_ptr->out_port_trie_root);
	  if (port_ptr == NULL)
	    {
      ...
      if (port_ptr->signal_ptr == NULL)
	{
	  if (trie_search (port_name, entity_ptr->in_port_trie_root) != NULL)
	    {
	      port_ptr->next =
		(t_port *) trie_search (signal_name,
	      ...
	      if (port_ptr->next == NULL)
		{
	      ...
	      port_ptr->signal_ptr = port_ptr->next->signal_ptr;
	      port_ptr->signal_ptr->value =
		lookup_RES[port_ptr->signal_ptr->value][port_ptr->port_value];
	    }
	}
      port_ptr->next = NULL;
      ...
      while (scanf (" %1[,]", temp_string_end_buffer) == 1)
	{
	  ...
	  if (port_ptr->signal_ptr == NULL)
	    {
	      if (trie_search (port_name, entity_ptr->in_port_trie_root) !=
		  NULL)
		{
		  port_ptr->next =
		    (t_port *) trie_search (signal_name,
		  ...
		  if (port_ptr->next == NULL)
		    {
		      printf
			("** ERROR:invalid port name for the port map\n");
		      exit (EXIT_FAILURE);
		    }
		}
	      else
  ...
  t_testcase_handle process_copy_handle;
  process_copy_handle =
    copy_testcase (architecture_ptr->architecture_testcase_handle,
		   port_map_trie_root, parent_signal_tail_address,
		   instance_name);
  if (process_copy_handle.process_list_head != NULL)
    {
      if (previous_process_ptr != NULL)
	{
	  ((t_process *) ((void *) (process_copy_handle.signal_trie_root)))->
	    next = previous_process_ptr->next;
